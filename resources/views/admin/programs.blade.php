    ...
    <table class="min-w-full text-sm text-slate-700 dark:text-slate-200">
        ...
        @forelse($programs as $program)
            <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/30">
                ...
                <td class="px-4 py-2 text-center">
                    <form method="POST" action="{{ secure_route('admin.programs.update', $program->id) }}" style="display:inline-block">
                        @csrf
                ...
                <td class="px-4 py-2 text-center">
                    <form method="POST" action="{{ secure_route('admin.programs.destroy', $program->id) }}" style="display:inline-block" onsubmit="return confirm(@js(__('messages.delete_program_confirm')))">
                        @csrf
                        @method('DELETE')
