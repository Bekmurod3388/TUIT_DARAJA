...
@section('title', __('messages.new_commission'))
...
<div class="mx-auto max-w-lg rounded-2xl border border-slate-200/80 bg-white/90 p-8 shadow-sm backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800/90">
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-white">{{ __('messages.new_commission') }}</h1>
    <form method="POST" action="{{ secure_url(route('admin.commissions.store', [], false)) }}">
        @csrf
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.chairman') }}</label>
            <input type="text" name="chairman" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required value="{{ old('chairman') }}">
        </div>
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.deputy') }}</label>
            <input type="text" name="deputy" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required value="{{ old('deputy') }}">
        </div>
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.secretary') }}</label>
            <input type="text" name="secretary" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required value="{{ old('secretary') }}">
        </div>
        <div class="mb-4">
            ...
            <select name="specalization_id" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required>
                ...
                @foreach($specalizations as $spec)
                    <option value="{{ $spec->id }}" @selected(old('specalization_id') == $spec->id)>{{ $spec->name }} ({{ $spec->code }})</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
        ...
        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">{{ __('messages.save_changes') }}</button>
