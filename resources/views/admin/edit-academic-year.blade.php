...
<div class="mx-auto max-w-lg rounded-2xl border border-slate-200/80 bg-white/90 p-8 shadow-sm backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800/90">
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-white">{{ __('messages.edit_academic_year') }}</h1>
    <form method="POST" action="{{ secure_route('admin.academic-years.update', $academicYear->id) }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.academic_year') }}</label>
            <input type="text" name="name" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required value="{{ old('name', $academicYear->name) }}" placeholder="2025/2026">
        </div>
        <div class="mb-6">
            ...
            <select name="semester" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" required>
                ...
                <option value="bahorgi" @selected(old('semester', $academicYear->semester) === 'bahorgi')>{{ __('messages.spring_semester') }}</option>
                <option value="kuzgi" @selected(old('semester', $academicYear->semester) === 'kuzgi')>{{ __('messages.fall_semester') }}</option>
            </select>
        </div>
        <label class="mb-6 flex items-center gap-2 text-slate-900 dark:text-white">
            <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('is_active', $academicYear->is_active))>
