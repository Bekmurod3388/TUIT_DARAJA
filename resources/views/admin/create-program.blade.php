...
@section('title', __('messages.new_program'))
...
<div class="mx-auto max-w-lg rounded-2xl border border-slate-200/80 bg-white/90 p-8 shadow-sm backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800/90">
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-white">{{ __('messages.new_program') }}</h1>
    @if ($errors->any())
        <div class="mb-4">
            <div class="font-semibold text-red-600">{{ __('messages.error_occurred') }}
                <ul>
    ...
    <form method="POST" action="{{ secure_route('admin.programs.store') }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="program-code" class="w-full rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-slate-700 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200" readonly>
        </div>
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.payment_amount_soum') }}</label>
            <input type="number" name="price" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" min="0" required>
        </div>
        <div class="mb-4">
            <label class="mb-1 block font-semibold text-slate-900 dark:text-white">{{ __('messages.short_description') }}</label>
            <textarea name="description" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" rows="3" required></textarea>
        </div>
